...
## Часть 1.
Сразу оговорюсь, что в качестве имени сотрудника для данного отдела с самой высокой зарплатой `name_highest_sal` я буду выводить имя и фамилию, ведь как мы знаем, людей с одним именем в этой жизни слишком много.
...
Раз мы делаем задание по теме оконных функций, я в этом задании их и использую (первый запрос). Но данное задание несложно выполнить с помощью группировки, поэтому я решил проверить результат своего первого запроса запросом с группировкой.

Итак, в запросе с оконными функциями выполняется соединение таблицы `SALES` с таблицей `GOODS` по условию для `DATE` и по полю `ID_GOOD` для получения поля `PRICE`, также соединение с `SHOPS` по `SHOPNUMBER` для получения `CITY` и `ADDRESS`. Данные бьются на секции (окна) по магазинам - полю `SHOPNUMBER`. После этого с помощью агрегатных функций `SUM` в каждой секции высчитываются искомые `SUM_QTY` и  `SUM_QTY_PRICE`. Раз мы посчитали логичным в результате видеть одну запись по каждому магазину, то я в `SELECT` делаю `DISTINCT` для отбора уникальных записей.

В запросе с группировкой идея аналогична. Делается группировка по магазинам: по полю `SHOPNUMBER` (также по `CITY` и `ADDRESS`, которые не влияют на группировку, но нужны, чтобы извлечь эти поля из группы). В каждой группе высчитываются искомые `SUM_QTY` и  `SUM_QTY_PRICE`. 

Для удобства сравнения результатов в двух запросах записи отсортированы по столбцу `SHOPNUMBER`.
...
В табличном выражении `get_is_final` рассчитывается поле is_final с помощью оператора `case`. Если пользователь после данного запроса больше ничего не искал, то `LEAD(ts) OVER w` будет `NULL`, следовательно, поле `ts_diff` будет `NULL` (это уже вычислено в `get_criteria`). Чтобы написать условие, заметим, что для первого и второго условия `is_final = 1`. Поэтому у меня это в коде реализовано через логическое или: `ts_diff IS NULL OR ts_diff > 180`.

После табличного выражения `get_is_final`, в котором определяется `is_final` для каждого запроса, делается финальный запрос, в котором выводятся данные о запросах, подходящих под необходимые условия.

На всякий случай оговорюсь, что когда речь идет об устройстве `android`, под этим я понимаю `devicetype`, а когда нужно именно само устройство (уникальное), то я работаю с `deviceid`.

Я оставляю в решении табличное выражение `get_is_final`, дабы улучшить читаемость и не дублировать один и тот же кусок кода.

Также в конце выполняется сортировка по пользователю `userid`, устройству `deviceid` и времени запроса `ts` для удобства просмотра результатов.
