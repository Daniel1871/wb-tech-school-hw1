...
Чтобы написать условие, заметим, что для первого и второго условия `is_final = 1`. Поэтому у меня это в коде реализовано через логическое или `ts_diff IS NULL OR ts_diff > 180`.
...
Вообще говоря `get_is_final` я написал, дабы улучшить читаемость и не дублировать один и тот же кусок кода.
